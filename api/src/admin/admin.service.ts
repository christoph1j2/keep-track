...
export class AdminService {
    ...
    async getStats(): Promise<any> {
        ...
        const [
            ...
            this.prisma.transaction.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
        ...
        const aiCategorizationRate = transactionCount > 0
        ...
        importJobs.forEach((job) => {
            if (job.status in importJobStats) {
                importJobStats[job.status as keyof typeof importJobStats] = job._count.status;
            }
    ...
    async getUserDetails(userId: string): Promise<any> {
        ...
        const [
            ...
            this.prisma.transaction.count({ where: { userId, isAiCategorized: true } }),
        ...
        const aiRate = transactionCount > 0 ? Math.round((aiCategorizedCount / transactionCount) * 100) : 0;
