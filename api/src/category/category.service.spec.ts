...
describe('CategoryService', () => {
  ...
  describe('update', () => {
    it('should update a category', async () => {
      ...
      expect(await service.update('user-1', '1', dto as any)).toEqual({
        ...category,
        ...dto,
      });
    ...
    it('should throw BadRequestException if category is its own parent', async () => {
      ...
      await expect(
        service.update('user-1', '1', { parentId: '1' } as any),
    ...
    it('should throw BadRequestException on circular dependency (A -> B -> A)', async () => {
      jest
        ...
        .mockResolvedValue({ id: '1', userId: 'user-1' } as any);
      mockPrismaService.category.findUnique.mockResolvedValue({
        parentId: '1',
      });

      await expect(
        service.update('user-1', '1', { parentId: '2' } as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('reorder', () => {
    it('should throw NotFoundException if a category is missing', async () => {
      ...
      await expect(
        service.reorder('user-1', { categories: [{ id: '1', order: 1 }] }),
