...
import { TextField, Stepper, Step, StepLabel, RadioGroup, FormControlLabel, Radio, Box, Select, MenuItem } from "@mui/material";
...
export function AddBudgetWizardModal({ onCancel }: AddBudgetWizardModalProps) {
  ...
  const baseSteps = budgetType === "simple" 
    ? [t("budgeting.wizard.stepType"), t("budgeting.wizard.stepDetails")] 
    : [t("budgeting.wizard.stepType"), t("budgeting.wizard.stepIncome"), t("budgeting.wizard.stepCategories")];
    
  const categorySteps = budgetType === "complex" 
    ? selectedCategories.map(id => {
        const cat = categories.find(c => c.id === id);
        return cat ? (cat.label.startsWith("default_categories.") ? t(cat.label) : cat.label) : t("common.category");
      })
    : [];
    
  const steps = budgetType === "simple" 
    ? baseSteps 
  ...
  const currentCategory = currentCategoryStepIndex >= 0 && currentCategoryStepIndex < selectedCategories.length 
    ? categories.find(c => c.id === selectedCategories[currentCategoryStepIndex]) 
    : null;
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <div className="flex flex-col gap-4 my-4 p-1 min-h-[16rem]">
        {step === 0 && (
          <div className="flex flex-col gap-4">
            ...
            <RadioGroup
              value={budgetType}
              onChange={(e) => setBudgetType(e.target.value as "simple" | "complex")}
              className="text-slate-800 dark:text-slate-200"
            >
              <FormControlLabel 
                value="simple" 
                control={<Radio sx={{ color: 'gray', '&.Mui-checked': { color: '#2563eb' } }} />} 
              ...
              renderValue={(selected) => (
                <div className="flex flex-wrap gap-1">
                  {selected.map((val) => {
                    ...
                    const label = c ? (c.label.startsWith("default_categories.") ? t(c.label) : c.label) : val;
                    return (
                      <span key={val} className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded dark:bg-blue-900 dark:text-blue-200">
                        {label}
                      </span>
                    );
                  })}
            ...
            >
              {expenseCategories.map((cat) => (
                <MenuItem key={cat.id} value={cat.id}>
                  {cat.label.startsWith("default_categories.") ? t(cat.label) : cat.label}
                </MenuItem>
              ))}
            </Select>
          </div>
        ...
        {budgetType === "complex" && currentCategory && (
          <div className="flex flex-col gap-1">
            ...
            <TextField
              fullWidth size="small" type="number"
              value={categoryExpenses[currentCategory.id] === undefined ? "" : categoryExpenses[currentCategory.id]}
              onChange={(e) => setCatExpense(currentCategory.id, e.target.value === "" ? "" : Number(e.target.value))}
              sx={{ "& .MuiInputBase-input": { color: "inherit" } }}
              className="text-slate-900 dark:text-slate-200"
            />
          </div>
