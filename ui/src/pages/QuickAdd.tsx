...
export function QuickAdd() {
    ...
    const hotbarTemplates = useMemo(() => templates.filter((template) => template.showInHotbar), [templates]);
    ...
    const handleDragEnd = (event: DragEndEvent, scope: "all" | "hotbar") => {
        ...
        const reorderedHotbar = arrayMove(hotbarTemplates, oldHotbarIndex, newHotbarIndex);
        // vytvor kopii noveho poradi hotbaru
        const hotbarQueue = [...reorderedHotbar];
        // projdi vsechny sablony a namichej nove poradi
        reorderTemplates(templates.map((template) => (
        ...
        <div className="p-2 h-full flex flex-col gap-6">
            ...
            <section className="bg-white dark:bg-slate-900 dark:border-slate-600 p-6 rounded-2xl shadow-sm border border-slate-100">
                ...
                {hotbarTemplates.length > 0 ? (
                    <DndContext collisionDetection={closestCenter} onDragEnd={(event) => handleDragEnd(event, "hotbar")}>
                        <SortableContext items={hotbarTemplates.map((template) => template.id)} strategy={horizontalListSortingStrategy}>
