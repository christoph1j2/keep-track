...
/**
 * @deprecated No longer used. Consider removing this function in future refactoring.
 */
export function generateMockTransactions(
  ...
  if (categories.length === 0) {
    console.error(
      "Nelze generovat transakce bez kategorií. Prosím, vytvořte nejdříve kategorie.",
    );
    return [];
  }
  ...
  for (let i = 0; i < 200; i++) {
    ...
    if (isIncome) {
      ...
      const prefix =
        expensePrefixes[Math.floor(Math.random() * expensePrefixes.length)];
    ...
    const newTransaction: Transaction = {
      ...
      userId: "mock-user", // Mock user ID
      originalAmount: amount,
      originalCurrency: "CZK",
      isAiCategorized: false,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      // bankReferenceId is optional and can be null for mock data
